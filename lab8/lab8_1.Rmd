...
It's untidy for the same reasons as the last example. Tidy data has to (1) have a each variable with its own
column, income level in this case and (2) observations are grouped by religion. 
...
This data isn't tidy because the column names are ovservations for a vairable and each observation does not
have its own row.
...
```{r}
qpcr_untidy <- read_csv("data/qp`cr_untidy.csv")
qpcr_untidy
```
