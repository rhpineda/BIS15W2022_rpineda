...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "deepskyblue4", bins=9)+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="blue", alpha  =0.6, color = "orange")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "deepskyblue4", alpha = 0.6, color = "black")+
  geom_density(color = "red")+
  labs(title = "Distribution of Homerange") +
  theme_gray()
```
...
table(quartiles)
#just another summary fxn
```

2. How many and which taxonomic classes are represented in each of the `range_category` classes?
```{r}
homerange <- homerange %>% 
mutate(range_category = case_when(log10.hra <= 3.65 ~ " very small range",
                                log10.hra > 3.65 & log10.hra <= 4.595 ~ "small range",
                                log10.hra > 4.595 & log10.hra <= 6.013 ~ "medium range",
                                log10.hra > 6.013 ~ "large range"))
```

3. Isolate the small `range_category` and plot the range of `log10.mass` by taxonomic class.
```{r}
homerange %>% 
  filter(range_category == "small range") %>%
