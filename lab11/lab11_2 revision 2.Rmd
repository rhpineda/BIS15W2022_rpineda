...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
  facet_grid(migratory_strategy~.)+ #Rows ~ Columns
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Mass by Diet and Migratory Strategy",
       x = NULL,
       y = "Log 10 Mass",
...
ecosphere %>% 
  ...
  geom_density()+
  facet_grid(.~habitat)#+
  #theme(axis.test.x = element_text(angle = 60, hjust = 1))
```

2. Try the same plot as #1 above, but include life_expectancy~habitat.
```{r}
ecosphere %>% 
  ggplot(aes(x=mean_eggs_per_clutch, color = "red")) + 
  geom_density()+
  facet_grid(life_expectancy~habitat, scales = "free")
```
```{r}
ecosphere %>% 
  ggplot(aes(x=mean_eggs_per_clutch, color = order)) + 
  geom_density()+
  facet_grid(life_expectancy~habitat, scales = "free")
```
